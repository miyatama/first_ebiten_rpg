...
func LoadTitleMovableMap() (map[util.MapPosition]bool, error) {
...
func LoadHouseMovableMap() (map[util.MapPosition]bool, error) {
	slog.Info("maps LoadMovableMap()")
	maps := make(map[util.MapPosition]bool)
	img, _, err := image.Decode(bytes.NewReader(images.HouseMovableMapImage))
	if err != nil {
		slog.Error("title_movable_map_image.png decode error")
		slog.String("error: {}", err.Error())
		return maps, err
